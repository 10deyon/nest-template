...
export const paginate = (
  ...
  return {
    links: {
      ...
      next:
        count > limit
          ? `${route}?limit=${limit}&page=${Number(page) + 1}`
          : null,
      last:
        count > limit
          ? `${route}?limit=${limit}&page=${count / Number(limit)}`
          : null,
    },
...
export const handleArraySearch = (
  data: (string | number)[],
...
export const handleFilterExact = (
  searchTerm: string,
  searchFieldName: string,
) => {
  return !!searchTerm ? { [searchFieldName]: searchTerm } : {};
};
...
export const datesFilterQuerys: (
  scheduleDate: string,
  dateFieldName: string,
) => { [key: string]: { $gt: Date; $lte: Date } } = (
  scheduleDate: string,
  dateFieldName: string,
) => {
  if (!scheduleDate) {
  ...
  const startDate: Date = new Date(
    Helpers.subtractDaysFromDate(0, scheduleDateQuery).setHours(0, 59, 59, 999),
  );
  const endDate: Date = new Date(
    Helpers.addDaysToDate(1, scheduleDateQuery).setHours(0, 59, 59, 999),
  );
  return {
    [dateFieldName]: {
      $gt: startDate,
