...
export class Helpers {
  ...
  static randomString(length = 5): string {
    ...
    for (
      let counter = 0, charsLength: number = chars.length;
      counter < length;
      counter += 1
    ) {
  ...
  static generateSerialString(
    ...
    for (
      let outputPosition = 1,
        runningRemindand: number = serialNumber,
        outputPositionWeight: number;
  ...
  static getFirstNameLastNameFromFullName(fullName: string): {
    firstName: string;
    lastName: string;
  } {
    const [firstName, ...lastNameArray] = String(fullName)
      .split(' ')
  ...
  static addSubtractDays(date: Date, noOfDays: number, add = true): string {
  ...
  static formatUppercaseString(oldString: string): string {
    const newString: string = String(oldString)
      .replace(/_/g, ' ')
      .toLowerCase();
